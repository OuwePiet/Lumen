...
export default function ViaPong(){
 ...
 return <article style={{border:"1px solid #285a3e",borderRadius:20,padding:20,marginTop:24,background:"linear-gradient(145deg,#07100b,#020504)",boxShadow:"0 0 40px rgba(70,210,130,.08)"}}>
  ...
  <div style={{display:"flex",gap:8,flexWrap:"wrap",margin:"14px 0"}}>{MODES.map(m=><button key={m} type="button" disabled={game.current.running} aria-pressed={mode===m} onClick={()=>setMode(m)} style={{padding:"8px 12px",borderRadius:999,border:`1px solid ${mode===m?"#7ee2a8":"#31513f"}`,background:mode===m?"#143222":"transparent",color:"white",fontWeight:800}}>{m}</button>)}</div>
