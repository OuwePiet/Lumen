...
export default function QuestGame(){
 ...
 function startSurprise(){setRouteIndex(v=>mode==="surprise"?(v+1)%surpriseRoutes.length:v);setMode("surprise");setStep(0)}
 function completeStep(){if(finished)return;const next=step+1;setStep(next);const repeatDaily=mode==="daily"&&completedToday;if(!repeatDaily)setState(v=>({...v,points:v.points+10}));if(next===totalSteps&&!repeatDaily)setState(v=>{if(mode!=="daily")return{...v,points:v.points+25};const nextStreak=v.lastDailyDate===yesterday()?v.streak+1:1;return{points:v.points+25,streak:nextStreak,lastDailyDate:today}})}
 return <section className={styles.game} aria-label="Playable VIA World Quest prototype">
  ...
  <article className={styles.questCard}><div className={styles.progressLine}><span>{mode==="daily"?"Daily Quest":"Surprise Route"}</span><span>{Math.min(step,totalSteps)} / {totalSteps}</span></div>
   {!finished?<><h2>{currentLabel}</h2><p>{mode==="daily"&&completedToday?"Today’s Daily Quest is already recorded. You may replay it, but no additional Daily points are awarded.":"This prototype records only local game progress. Future versions will connect each step to verified VIA Discovery destinations."}</p><button className={styles.primary} type="button" onClick={completeStep}>{mode==="daily"&&completedToday?"Replay step":"Mark step complete · +10 points"}</button></>:<div className={styles.finished}><span aria-hidden="true">◆</span><h2>Quest complete</h2><p>{mode==="daily"&&completedToday?"Today’s Daily Quest is recorded. Come back tomorrow to continue your streak.":"You earned the completion bonus. Try another route."}</p><button className={styles.primary} type="button" onClick={mode==="daily"?startSurprise:startDaily}>Play another route</button></div>}
  </article>
  <p className={styles.note}>VIA Points and streaks are local, non-transferable game progress only. They have no cash value and do not entitle a player to DESO, Diamonds, tokens, payouts or any other financial reward.</p>
 </section>
}
