...
export default function PostComposer({ parentStakeID = "", compact = false, onDone }: PostComposerProps) {
  ...
  async function preparePost() {
    ...
    setStatus("preparing")
    setMessage(isReply ? "Preparing the exact DeSo reply transaction…" : "Preparing the exact DeSo post transaction…")
    setFeeNanos(null)
    ...
    try {
      const response = await fetch("/api/via/social/post", {
        ...
        headers: { "Content-Type": "application/json" },
        cache: "no-store",
        body: JSON.stringify({ action: "prepare", publicKey: session.publicKey, body, parentStakeID }),
      })
      ...
      setMessage(isReply ? "Review and approve the exact reply in DeSo Identity. VIA will not submit it without that approval." : "Review and approve the exact post in DeSo Identity. VIA will not submit it without that approval.")
    } catch {
      setStatus("error")
      setMessage(isReply ? "The reply transaction could not be prepared. Nothing was posted." : "The post transaction could not be prepared. Nothing was posted.")
    }
  }

  if (!session) {
    return (
      <div className={`${compact ? "mt-3" : "mt-4"} rounded-xl border border-zinc-800 bg-black/30 p-4 text-sm text-zinc-500`}>
        Connect through the DeSo participation gate before {isReply ? "replying" : "composing a public post"}.
      </div>
    )
  }
  ...
  return (
    <div className={`${compact ? "mt-3" : "mt-5"} rounded-2xl border border-green-900/60 bg-black/35 p-4`}>
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-green-400">{isReply ? "Released write action · DeSo reply" : "Released write action · DeSo post"}</p>
      ...
      <textarea
        id={isReply ? `via-reply-${parentStakeID}` : "via-post-body"}
        value={body}
        onChange={(event) => { setBody(event.target.value); if (status === "done" || status === "error") { setStatus("idle"); setMessage("") } }}
        maxLength={MAX_POST_LENGTH}
        rows={compact ? 3 : 5}
        placeholder={isReply ? "Write a public reply on DeSo…" : "What would you like to share on DeSo?"}
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        ...
        >
          {status === "preparing" ? "Preparing…" : status === "awaiting-approval" ? "Awaiting DeSo approval…" : status === "submitting" ? "Submitting…" : isReply ? "Review in DeSo & reply" : "Review in DeSo & post"}
        </button>
