...
export default function PostComposer({ parentStakeID = "", compact = false, onDone }: PostComposerProps) {
  ...
  const canPrepare = Boolean(session && hasContent && body.length <= MAX_POST_LENGTH && !mediaInvalid && !busy)
  ...
  return (
    <div className={`${compact ? "mt-3" : "mt-5"} rounded-2xl border border-green-900/60 bg-black/35 p-4`}>
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-green-400">{isReply ? "Released write action · DeSo reply" : "Released write action · DeSo post + media URL"}</p>
      ...
      {!compact ? <div className="mt-4 rounded-xl border border-zinc-800 bg-zinc-950/70 p-3">
        <p className="text-sm font-medium text-zinc-200">Media already stored on DeSo/IPFS or another durable HTTPS location</p>
        <p className="mt-1 text-xs leading-5 text-zinc-500">VIA stores no media file here. Only the HTTPS media URL is placed in the DeSo post transaction. Direct DeSo image upload follows as a separate JWT-protected step.</p>
