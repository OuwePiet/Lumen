      ...
      {visiblePosts.length > 0 ? <div className="mt-5 space-y-3">{visiblePosts.map((post) => {
        ...
        return <article key={post.postHash} className="rounded-[12px] border border-zinc-800/80 bg-black/30 p-4">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-zinc-500">
            {session ? <LikeButton postHash={post.postHash} initialCount={post.likeCount} /> : <span>{post.likeCount} likes</span>}
            <span>{post.diamondCount} Diamonds</span>
            <span>{post.commentCount} replies</span>
            {session ? <RepostButton postHash={post.postHash} initialCount={totalReposts} /> : <span>{totalReposts} reposts</span>}
            {post.isNft ? <span className="text-[#8fd4a9]">NFT</span> : null}
            {session ? <FollowButton followedPublicKey={post.publicKey} /> : <span className="text-zinc-600">Follow · DeSo login</span>}
            {session ? <button type="button" onClick={() => setReplyingTo(isReplying ? null : post.postHash)} className="rounded-full border border-green-900/70 px-3 py-1 text-green-300 hover:border-green-700">{isReplying ? "Close reply" : "Reply"}</button> : <span className="text-zinc-600">Reply · DeSo login</span>}
          </div>
