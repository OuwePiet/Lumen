...
const futureIdeas = [
  { title: "External creator collectible intake", text: "Evaluate a paid public creator route for preparing media and collectible metadata without operating a VIA blockchain or node. Any external item shown in VIA must carry a clear External collectible label; native on-chain items keep the DeSo NFT label. A future Mint on DeSo action may only appear after creator consent, media/metadata validation, an explicit live DeSo cost quote and final approval; purchase and ownership require their own verified flow." },
