...
export async function POST(request: Request) {
  ...
  if (body.action === "prepare") {
    ...
    const isUnlike = body.isUnlike === true
    ...
    try {
      const response = await fetchDeSo("create-like-stateless", {
        method: "POST",
    ...
    } catch { return noStore({ ok: false, error: "DESO_PREPARE_UNAVAILABLE" }, 503) }
  }
  ...
  if (body.action === "submit") {
    ...
    } catch { return noStore({ ok: false, error: "DESO_SUBMIT_UNAVAILABLE" }, 503) }
  }
