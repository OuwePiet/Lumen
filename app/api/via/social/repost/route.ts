...
export async function POST(request: Request) {
  ...
  if (body.action === "prepare") {
    ...
    if (!quote && (imageUrls.length > 0 || videoUrls.length > 0)) return noStore({ ok: false, error: "QUOTE_TEXT_REQUIRED" }, 400)
    ...
    try {
      const response = await fetchDeSo("submit-post", {
        ...
        body: JSON.stringify({
          ...
          Title: "",
          BodyObj: { Body: quote, ImageURLs: imageUrls, VideoURLs: videoUrls },
      ...
      return noStore({ ok: true, transactionHex, feeNanos: typeof feeNanos === "number" && Number.isFinite(feeNanos) ? feeNanos : null, media: { images: imageUrls.length, videos: videoUrls.length } })
    } catch {
