...
export async function POST(request: Request) {
  ...
  if (body.action === "prepare") {
    ...
    const amountNanos = body.amountNanos
    const confirmed = body.confirmed === true
    if (!validPublicKey(senderPublicKey)) return noStore({ ok: false, error: "INVALID_SENDER_PUBLIC_KEY" }, 400)
    if (!validPublicKey(recipientPublicKey)) return noStore({ ok: false, error: "INVALID_RECIPIENT_PUBLIC_KEY" }, 400)
    if (!Number.isSafeInteger(amountNanos) || (amountNanos as number) < 1) return noStore({ ok: false, error: "INVALID_AMOUNT" }, 400)
    ...
    try {
      const response = await fetchDeSo("send-deso", {
        method: "POST",
      ...
      return noStore({ ok: true, transactionHex, amountNanos, feeNanos: typeof feeNanos === "number" && Number.isFinite(feeNanos) ? feeNanos : null })
    } catch {
