...
export async function POST(request: Request) {
  ...
  if (body.action === "prepare") {
    if (!validPublicKey(body.publicKey)) return json({ ok: false, error: "INVALID_PUBLIC_KEY" }, 400)
    ...
    try {
      const response = await fetchDeSo("create-nft-bid", {
        ...
        body: JSON.stringify({
          UpdaterPublicKeyBase58Check: body.publicKey,
      ...
      if (!response.ok) return json({ ok: false, error: "DESO_WITHDRAW_BID_PREPARE_REJECTED" }, 502)
      ...
      return json({ ok: false, error: "DESO_WITHDRAW_BID_PREPARE_UNAVAILABLE" }, 503)
    }
  }
  ...
  if (body.action === "submit") {
    ...
    try {
      ...
      if (!response.ok) return json({ ok: false, error: "DESO_WITHDRAW_BID_SUBMIT_REJECTED" }, 502)
      return json({ ok: true, transaction: await response.json() as Record<string, unknown> })
    } catch {
      return json({ ok: false, error: "DESO_WITHDRAW_BID_SUBMIT_UNAVAILABLE" }, 503)
    }
  }
