...
export async function POST(request: Request) {
  ...
  if (body.action === "prepare") {
    if (!validPublicKey(body.ownerPublicKey)) return json({ ok: false, error: "INVALID_OWNER" }, 400)
    if (!validPublicKey(body.bidderPublicKey)) return json({ ok: false, error: "INVALID_BIDDER" }, 400)
    if (!validPostHash(body.postHash)) return json({ ok: false, error: "INVALID_POST_HASH" }, 400)
    if (!validPositiveSafeInteger(body.serialNumber)) return json({ ok: false, error: "INVALID_SERIAL_NUMBER" }, 400)
    if (!validPositiveSafeInteger(body.bidAmountNanos)) return json({ ok: false, error: "INVALID_BID_AMOUNT" }, 400)
    if (body.hasUnlockable === true) return json({ ok: false, error: "UNLOCKABLE_ACCEPT_REQUIRES_ENCRYPTION" }, 400)
    ...
    try {
      const response = await fetchDeSo("accept-nft-bid", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          UpdaterPublicKeyBase58Check: body.ownerPublicKey,
          NFTPostHashHex: String(body.postHash).toLowerCase(),
          SerialNumber: body.serialNumber,
          BidderPublicKeyBase58Check: body.bidderPublicKey,
          BidAmountNanos: body.bidAmountNanos,
          EncryptedUnlockableText: "",
          MinFeeRateNanosPerKB: minFeeRate,
          TransactionFees: [],
        }),
      })
      if (!response.ok) return json({ ok: false, error: "DESO_ACCEPT_BID_PREPARE_REJECTED" }, 502)
      ...
      return json({ ok: false, error: "DESO_ACCEPT_BID_PREPARE_UNAVAILABLE" }, 503)
    }
  }
  ...
  if (body.action === "submit") {
    ...
    try {
      ...
      if (!response.ok) return json({ ok: false, error: "DESO_ACCEPT_BID_SUBMIT_REJECTED" }, 502)
      return json({ ok: true, transaction: await response.json() as Record<string, unknown> })
    } catch {
      return json({ ok: false, error: "DESO_ACCEPT_BID_SUBMIT_UNAVAILABLE" }, 503)
    }
  }
