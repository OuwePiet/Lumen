...
export async function POST(request: Request) {
  ...
  if (body.action === "prepare") {
    ...
    const isForSale = body.isForSale
    const minBidAmountNanos = body.minBidAmountNanos
    const isBuyNow = body.isBuyNow
    const buyNowPriceNanos = body.buyNowPriceNanos
    ...
    if (typeof isForSale !== "boolean" || typeof isBuyNow !== "boolean") return json({ ok: false, error: "INVALID_SALE_MODE" }, 400)
    if (!validNonNegativeSafeInteger(minBidAmountNanos) || !validNonNegativeSafeInteger(buyNowPriceNanos)) return json({ ok: false, error: "INVALID_PRICE" }, 400)
    if (!isForSale && (minBidAmountNanos !== 0 || isBuyNow || buyNowPriceNanos !== 0)) return json({ ok: false, error: "INVALID_NOT_FOR_SALE_VALUES" }, 400)
    if (!isBuyNow && buyNowPriceNanos !== 0) return json({ ok: false, error: "INVALID_BUY_NOW_PRICE" }, 400)
    if (isBuyNow && buyNowPriceNanos < minBidAmountNanos) return json({ ok: false, error: "BUY_NOW_BELOW_MIN_BID" }, 400)
    ...
    try {
      const response = await fetchDeSo("update-nft", {
        method: "POST",
      ...
      if (!response.ok) return json({ ok: false, error: "DESO_NFT_UPDATE_PREPARE_REJECTED" }, 502)
      const data = await response.json() as Record<string, unknown>
      if (!validHex(data.TransactionHex)) return json({ ok: false, error: "INVALID_PREPARED_TRANSACTION" }, 502)
      return json({ ok: true, transactionHex: data.TransactionHex, feeNanos: typeof data.FeeNanos === "number" && Number.isFinite(data.FeeNanos) ? data.FeeNanos : null })
    } catch {
      return json({ ok: false, error: "DESO_NFT_UPDATE_PREPARE_UNAVAILABLE" }, 503)
    }
  }
  ...
  if (body.action === "submit") {
    ...
    try {
      ...
      if (!response.ok) return json({ ok: false, error: "DESO_NFT_UPDATE_SUBMIT_REJECTED" }, 502)
      return json({ ok: true, transaction: await response.json() as Record<string, unknown> })
    } catch {
      return json({ ok: false, error: "DESO_NFT_UPDATE_SUBMIT_UNAVAILABLE" }, 503)
    }
  }
