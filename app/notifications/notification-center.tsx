  ...
  function toggleCategory(id: Category) {
    if (id === "diamond1") {
      setActiveCategories((current) => {
        const enabled = current.includes("diamond1") && current.includes("diamondMany")
        return enabled ? current.filter((value) => value !== "diamond1" && value !== "diamondMany") : [...current.filter((value) => value !== "diamond1" && value !== "diamondMany"), "diamond1", "diamondMany"]
      })
      return
    }
    if (id === "all") {
      ...
      <div className="sticky top-[73px] z-20 border-b border-zinc-800 bg-zinc-950/95 px-3 py-3 backdrop-blur sm:px-4" aria-label={copy.filters}>
        <div className="flex flex-wrap gap-2">
          {categories.filter((option) => option.id !== "diamondMany").map((option) => {
            ...
            const count = option.id === "all" ? items.length : option.id === "diamond1" ? items.filter((item) => ["diamond1", "diamondMany"].includes(categoryOf(item))).length : items.filter((item) => categoryOf(item) === option.id).length
