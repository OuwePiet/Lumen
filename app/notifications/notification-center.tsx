...
export default function NotificationCenter({ language }: { language: ViaLanguage }) {
  ...
  return (
    <section className={`${expandedView ? "max-h-[calc(100vh-8rem)]" : "max-h-[76vh]"} overflow-y-auto rounded-2xl border border-zinc-800 bg-zinc-950/60`} aria-labelledby="notification-center-heading">
      <div className="sticky top-0 z-20 flex flex-wrap items-center justify-between gap-1.5 border-b border-zinc-800 bg-zinc-950/95 px-2 py-1.5 backdrop-blur sm:gap-3 sm:px-5 sm:py-4">
        <div>
          <h2 id="notification-center-heading" className="text-xs font-medium text-zinc-400 sm:text-xl sm:font-semibold sm:text-white">{copy.heading}<span className="ml-1 text-[#8fd4a9] sm:hidden">Notifications</span></h2>
      ...
      <div className="mt-1 divide-y divide-zinc-800 sm:mt-2">
        ...
        {visible.map((item, index) => {
          ...
          return <article key={rowKey} className={`grid grid-cols-[38px_minmax(0,1fr)] gap-2 px-3 py-3 transition sm:grid-cols-[46px_minmax(0,1fr)_auto] sm:gap-3 sm:px-5 sm:py-4 ${unread ? "bg-[#0b1510]/70" : "bg-black/10"}`}>
