    ...
    <main style={styles.main}>
      <div style={styles.shell}>
        ...
        <section style={styles.grid} aria-label="World regions">
          {regions.map((region) => (
            <article key={region} style={styles.card}>
              <h2 style={styles.cardTitle}>{region}</h2>
              <p style={styles.cardText}>
                Region labels are navigation only. VIA does not claim that a creator is located here unless suitable public evidence exists.
              </p>
              <div style={styles.actions}>
                <Link href="/radio" style={styles.link}>Explore radio</Link>
        ...
        <div style={styles.notice}>
          Around the World becomes richer only when VIA can verify a suitable public source. No private location inference and no paid placement disguised as organic discovery.
        </div>
      </div>
    </main>
  )
