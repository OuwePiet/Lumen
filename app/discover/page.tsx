  ...
  return (
    <main style={styles.main}>
      <div style={styles.shell}>
        ...
        <p style={styles.lead}>One VIA hub for creators, NFTs, world radio and daily discovery. The foundation stays low-cost and read-only while public data and DeSo capabilities are verified step by step.</p>
        <div style={styles.safety}>Discovery is not authority: opening these routes never proves identity or location and never silently signs, spends, follows, likes, transfers assets, or sends Diamonds. Saving a route is also explicit and stays in this browser.</div>
        ...
        <section style={styles.grid} aria-label="World Discovery sections">
          {sections.map((section) => (
            <article key={section.title} style={styles.card}>
              <div style={styles.cardTop}><h2 style={styles.cardTitle}>{section.title}</h2><span style={styles.badge}>{section.status}</span></div>
