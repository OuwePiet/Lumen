...
export default function DiscoverPage() {
  ...
  const creatorKindTargets = creatorKinds.map((kind) => ({ kind, window: creatorWindows.find((window) => window.kind === kind)!, count: creatorWindows.filter((window) => window.kind === kind).length }))
  ...
  return (
    <main style={styles.main}>
      <div style={styles.shell}>
        ...
        <div style={{ ...styles.nav, marginBottom: "12px" }} aria-label="Creator scope shortcuts">{creatorKindTargets.map(({ kind, window, count }) => <a key={kind} href={`#${creatorWindowAnchor(window)}`} style={styles.scopeLink}>{kind.toUpperCase()} {count > 1 ? `(${count})` : ""}</a>)}</div>
