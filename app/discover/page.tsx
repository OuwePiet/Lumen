  ...
  return (
    <main style={styles.main}>
      <div style={styles.shell}>
        ...
        <div style={{ ...styles.nav, marginBottom: "12px" }} aria-label="Creator scope shortcuts"><a href="/discover" style={styles.scopeLink} aria-current={!selectedKind ? "page" : undefined}>ALL</a>{creatorKindTargets.map(({ kind, window, count, openCount }) => <a key={kind} href={`?kind=${kind}#${creatorWindowAnchor(window)}`} style={styles.scopeLink} aria-current={selectedKind === kind ? "page" : undefined} aria-label={`${kind} creator scope: ${openCount} of ${count} windows open`} data-creator-kind={kind} data-open-windows={openCount} data-total-windows={count} data-testid={`creator-scope-${kind}`}>{kind.toUpperCase()} {count > 1 ? `(${openCount}/${count} open)` : openCount ? "(open)" : "(window)"}</a>)}</div>
        <p style={{ ...styles.cardText, marginBottom: "12px" }} aria-live="polite">{selectedKind ? `Showing ${selectedKind.toUpperCase()} · ${orderedCreatorWindows.length} creator window${orderedCreatorWindows.length === 1 ? "" : "s"}` : `Showing ALL · ${orderedCreatorWindows.length} creator windows`}</p>
