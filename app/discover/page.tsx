  ...
  return (
    <main style={styles.main}>
      <div style={styles.shell}>
        ...
        <div style={{ ...styles.nav, marginBottom: "12px" }} aria-label="Creator scope shortcuts"><Link href={openOnly ? "/discover?availability=open" : "/discover"} style={styles.scopeLink} aria-current={!selectedKind ? "page" : undefined} title="Show all creator windows">ALL</Link>{creatorKindTargets.map(({ kind, window, count, openCount }) => <Link key={kind} href={`/discover?kind=${kind}${openOnly ? "&availability=open" : ""}#${creatorWindowAnchor(window)}`} style={styles.scopeLink} aria-current={selectedKind === kind ? "page" : undefined} title={`Show ${kind} creator windows`} aria-label={`${kind} creator scope: ${openCount} of ${count} windows open`} data-creator-kind={kind} data-open-windows={openCount} data-total-windows={count} data-testid={`creator-scope-${kind}`}>{kind.toUpperCase()} {count > 1 ? `(${openCount}/${count} open)` : openCount ? "(open)" : "(window)"}</Link>)}</div>
        ...
        <p style={{ ...styles.cardText, marginBottom: "12px" }} aria-live="polite">{selectedKind ? `Showing ${selectedKind.toUpperCase()} · ${openOnly ? "OPEN NOW" : "ALL WINDOWS"} · ${orderedCreatorWindows.length} creator window${orderedCreatorWindows.length === 1 ? "" : "s"} · ${visibleOpenCreatorWindows} open` : `Showing ALL · ${openOnly ? "OPEN NOW" : "ALL WINDOWS"} · ${orderedCreatorWindows.length} creator windows · ${visibleOpenCreatorWindows} open`}</p>
        <section style={styles.grid} aria-label={selectedKind ? `${selectedKind} creator discovery, ${openOnly ? "open now" : "all windows"}: ${orderedCreatorWindows.length} windows, ${visibleOpenCreatorWindows} open` : `All creator discovery, ${openOnly ? "open now" : "all windows"}: ${orderedCreatorWindows.length} windows, ${visibleOpenCreatorWindows} open`}>
