...
type Copy = {
  ...
  regex: Array<[RegExp, (...parts: string[]) => string]>
}

const COMMON: Record<ViaLanguage, Copy> = {
  Dutch: {
    exact: {
...
function translated(source: string, copy: Copy) {
  ...
  for (const [pattern, replace] of copy.regex) {
...
function translateRoot(root: HTMLElement, language: ViaLanguage) {
  ...
  root.querySelectorAll("input, select, button, [aria-label]").forEach((element) => {
