...
export default async function ReceivedBidsPage({searchParams}:{searchParams:Promise<{publicKey?:string}>}){
 ...
 return <main className="mx-auto min-h-screen max-w-5xl px-5 py-10 text-zinc-100">
  <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-green-500">VIA Marketplace</p><h1 className="mt-2 text-3xl font-semibold">Received bids</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-400">Active native DeSo bids on NFT editions owned by this account. Open an NFT to review and accept a bid with VIA's guarded transaction flow.</p></div><Link href={"/market"+q} className="text-sm text-green-400">Back to Market</Link></div>
  {!publicKey?<form className="mt-8 rounded-xl border border-zinc-800 bg-black/20 p-4"><label className="text-sm text-zinc-300">DeSo public key<input name="publicKey" required placeholder="BC1…" className="mt-2 block w-full rounded-lg border border-zinc-800 bg-black px-3 py-2 text-sm"/></label><button className="mt-3 rounded-lg border border-green-800 px-4 py-2 text-sm font-semibold text-green-300">Show received bids</button></form>:error?<p className="mt-8 text-sm text-amber-300">{error}</p>:rows.length===0?<p className="mt-8 rounded-xl border border-zinc-800 p-5 text-sm text-zinc-400">No active received NFT bids found.</p>:<div className="mt-8 grid gap-3">{rows.map((r,i)=><Link key={r.hash+":"+r.serial+":"+r.bidder+":"+i} href={"/nft/"+r.hash} className="rounded-xl border border-zinc-800 bg-black/20 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-semibold">{title(r.post.Body)}</p><p className="mt-1 text-xs text-zinc-500">{r.post.ProfileEntryResponse?.Username?"@"+r.post.ProfileEntryResponse.Username+" · ":""}Edition #{r.serial} · {shortKey(r.bidder)}</p></div><p className="text-sm font-semibold text-green-300">{deso(r.amount)} DESO</p></div></Link>)}</div>}
 </main>
}
