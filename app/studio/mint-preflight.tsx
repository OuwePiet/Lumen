  ...
  return (
    <section className="mb-4 rounded-[14px] border border-zinc-800/80 bg-zinc-950/55 p-5 sm:p-6" aria-labelledby="mint-preflight-heading">
      ...
      {quoteUsable ? <div role="status" aria-live="polite" className="mt-4 rounded-[11px] border border-[#8fd4a9]/25 bg-[#8fd4a9]/5 px-4 py-3 text-sm leading-6 text-[#b8e5c8]">Quote safety checks passed: supported contract, active Identity matched, quote owner matched, device clock acceptable and quote still fresh. This is preflight readiness only; no mint has been signed or submitted.</div> : null}\n      {result?.resolved ? <div className="mt-4 rounded-[12px] border border-zinc-800/80 bg-black/25 p-4"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-zinc-500">Preflight status</p><span role="status" aria-live="polite" className={`inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs font-semibold ${quoteUsable ? "border-[#8fd4a9]/35 text-[#9adbb2]" : "border-red-900/60 text-red-200"}`}>{quoteUsable ? "Fresh · account matched" : `Blocked · ${quoteReadinessReason}`}<span aria-hidden="true" className={`font-mono text-[10px] ${quoteUsable ? "text-[#8fd4a9]/70" : "text-red-300/60"}`}>{quoteReadinessCode}</span></span></div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-[#8fd4a9]">Mint terms confirmed by DeSo preflight</p><div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div><p className="text-xs uppercase tracking-[0.12em] text-zinc-500">Copies</p><p className="mt-1 text-sm text-zinc-200">{result.mint?.numCopies ?? "—"}</p></div><div><p className="text-xs uppercase tracking-[0.12em] text-zinc-500">Sale</p><p className="mt-1 text-sm text-zinc-200">{result.mint?.isForSale ? (result.mint?.isBuyNow ? "Buy Now" : "Auction / bids") : "Not for sale"}</p></div><div><p className="text-xs uppercase tracking-[0.12em] text-zinc-500">Creator royalty</p><p className="mt-1 text-sm text-zinc-200">{typeof result.mint?.creatorRoyaltyBasisPoints === "number" ? `${(result.mint.creatorRoyaltyBasisPoints / 100).toFixed(2)}%` : "—"}</p></div><div><p className="text-xs uppercase tracking-[0.12em] text-zinc-500">Unlockable</p><p className="mt-1 text-sm text-zinc-200">{result.mint?.hasUnlockable ? "Yes" : "No"}</p></div><div><p className="text-xs uppercase tracking-[0.12em] text-zinc-500">Coin royalty</p><p className="mt-1 text-sm text-zinc-200">{typeof result.mint?.coinRoyaltyBasisPoints === "number" ? `${(result.mint.coinRoyaltyBasisPoints / 100).toFixed(2)}%` : "—"}</p></div><div><p className="text-xs uppercase tracking-[0.12em] text-zinc-500">Minimum bid</p><p className="mt-1 text-sm text-zinc-200">{result.mint?.isForSale ? nanos(result.mint?.minBidAmountNanos) : "Not applicable"}</p></div><div><p className="text-xs uppercase tracking-[0.12em] text-zinc-500">Buy Now price</p><p className="mt-1 text-sm text-zinc-200">{result.mint?.isBuyNow ? nanos(result.mint?.buyNowPriceNanos) : "Not applicable"}</p></div><div><p className="text-xs uppercase tracking-[0.12em] text-zinc-500">Post hash</p><p className="mt-1 break-all text-xs text-zinc-300">{result.mint?.nftPostHashHex ?? "—"}</p></div></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div><p className="text-xs uppercase tracking-[0.12em] text-zinc-500">Quote valid until</p><p className="mt-1 text-sm text-zinc-200">{result.expiresAt ? new Date(result.expiresAt).toLocaleTimeString() : "Unavailable"}{!quoteExpired && quoteSecondsLeft > 0 ? ` · ${Math.floor(quoteSecondsLeft / 60)}:${String(quoteSecondsLeft % 60).padStart(2, "0")} left` : ""}</p></div><div><p className="text-xs uppercase tracking-[0.12em] text-zinc-500">Quote created</p><p className="mt-1 text-sm text-zinc-200">{result.createdAt ? new Date(result.createdAt).toLocaleTimeString() : "Unavailable"}</p></div><div><p className="text-xs uppercase tracking-[0.12em] text-zinc-500">Server TTL</p><p className="mt-1 text-sm text-zinc-200">{typeof result.quoteTtlSeconds === "number" ? `${result.quoteTtlSeconds} seconds` : "Unavailable"}</p></div><div><p className="text-xs uppercase tracking-[0.12em] text-zinc-500">Server clock</p><p className="mt-1 text-sm text-zinc-200">{typeof result.serverNow === "number" ? new Date(result.serverNow).toLocaleTimeString() : "Unavailable"}</p></div><div><p className="text-xs uppercase tracking-[0.12em] text-zinc-500">Device clock difference</p><p className="mt-1 text-sm text-zinc-200">{typeof result.serverNow === "number" ? `${Math.round(serverClockOffset / 1000)} seconds` : "Unavailable"}</p></div><div><p className="text-xs uppercase tracking-[0.12em] text-zinc-500">Quote contract</p><p className={`mt-1 text-sm ${quoteContractSupported ? "text-[#9adbb2]" : "text-red-200"}`}>{typeof result.quoteContractVersion === "number" ? `V${result.quoteContractVersion} · ${quoteContractSupported ? "supported" : "unsupported"}` : "Unavailable"}</p></div><div><p className="text-xs uppercase tracking-[0.12em] text-zinc-500">Quote reference</p><p className="mt-1 break-all font-mono text-xs text-zinc-400">{result.quoteId ?? "Unavailable"}</p></div>
